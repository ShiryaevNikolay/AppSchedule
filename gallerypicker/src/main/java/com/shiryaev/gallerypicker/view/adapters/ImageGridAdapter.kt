package com.shiryaev.gallerypicker.view.adapters
...
import com.shiryaev.gallerypicker.utils.DateUtil
import com.shiryaev.gallerypicker.utils.RunOnUiThread
import com.shiryaev.gallerypicker.utils.scroll.FastScrollRecyclerView
        ...
        doAsync {
            RunOnUiThread(ctx).safely {
                try {
                    ...
                    Glide.with(ctx).load(mimageList[holder.adapterPosition].photoUri).apply(
                        RequestOptions().centerCrop().override(100)).transition(
                        DrawableTransitionOptions.withCrossFade()).listener(requestListener).into(holder.image)
                } catch (e: Exception) {
                }
            }
        }
